...
contract ComposableStrategy is IStrategy, ICHIModuleCommon, Composable {
    ...
    modifier onlyToken() {
        require(
            msg.sender == oneToken,
            "StrategyCommon: initialize from oneToken instance"
        );
        _;
    }
        ...
        if (msg.sender != oneToken) {
            if (msg.sender != IOneTokenV1Base(oneToken).controller()) {
                require(
                    ...
                    "StrategyCommon: not token controller or owner."
                );
            }
        }
        _;
    ...
    constructor(
        ...
        require(
            oneToken_ != NULL_ADDRESS,
            "StrategyCommon: oneToken cannot be NULL"
        );
        require(
            IOneTokenFactory(IOneTokenV1Base(oneToken_).oneTokenFactory())
                .isOneToken(oneToken_),
            "StrategyCommon: oneToken is unknown"
        );
        oneToken = oneToken_;

        registerFunction("init()", true);
        registerFunction("execute()", true);
        registerFunction("setAllowance(address,uint256)", true);
        registerFunction("closeAllPositions()", true);
        registerFunction("closePositions(address)", true);
        registerFunction("toVault(address,uint256)", true);
        registerFunction("fromVault(address,uint256)", true);
     ...
     */
    function init() external virtual override onlyToken {
     ...
     */
    function setAllowance(address token, uint256 amount)
        external
        override
        strategyOwnerTokenOrController
    {
     ...
     */
    function closePositions(address token)
        public
        virtual
        override
        strategyOwnerTokenOrController
     ...
     */
    function toVault(address token, uint256 amount)
        external
        override
        strategyOwnerTokenOrController
    {
    ...
    /**
     ...
     @param amount the amount of tokens to send
     */
    function fromVault(address token, uint256 amount)
        external
        override
        strategyOwnerTokenOrController
    {
